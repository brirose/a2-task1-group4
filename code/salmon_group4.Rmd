...
```{r timeseries}
# Bri, I am saving your df if that's ok - so I can use it later on!
salmon_longer <- willamette_salmon %>%  
  replace(is.na(.), 0) %>%  # replace na with 0
  pivot_longer(coho:steelhead, # consolidate species to one column
               names_to = "species",
...
#ggplot(data = salmon_longer, aes(x = date, y = counts, color = species)) + # make ggplot
```
...
Issues I'm having: 
* I think the "year" variable I'm using is not a date type? Bri's code for formatting the x-axis breaks isn't working for me.; yeah its not of date class; i converted it, but what you had also works (just remove the `mutate`)
* How to get the facets labeled with the proper names of the species?
...
* Once we collapse the data by year, there do not appear to be any common trends in the data across species.
* As seen in the original time series, Steelhead were sighted in greater numbers than the two Coho species across the entire study period, suggesting that the population is more abundant or that these fish are easier to count.  
* However, Coho fish were seen in greater abundance, particularly in the last two years of the study period, which could suggest a population increase. Jack Coho fish seem to be present in very low numbers, which could be cause for concern.
